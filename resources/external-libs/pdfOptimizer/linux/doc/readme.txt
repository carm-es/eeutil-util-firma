...
bin\php*_pdftools.dll     PHP 5.6/7.0 PdfTools extensions
...
doc\*_doc.php             PHP extension documentation
...
4. For use with PHP, add a new INI entry "extension=php70_pdftools.dll" to the php.ini,
   make sure the Pdf2PdfAPI.dll is listed in the PATH variable.
5. Install a license key using the license manager tool (also required for evaluation)
...
bin/php*_pdftools.so      PHP 5.6/7.0 PdfTools extensions (Linux only)
