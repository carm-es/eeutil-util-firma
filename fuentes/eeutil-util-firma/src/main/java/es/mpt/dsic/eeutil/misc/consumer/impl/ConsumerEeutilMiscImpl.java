...
public class ConsumerEeutilMiscImpl {
  ...
  protected static final Log logger = LogFactory.getLog(ConsumerEeutilMiscImpl.class);
  ...
  public void configure() throws InSideException {
    if (miscWs == null) {
      ...
      try {
        ...
        EeUtilServiceImplService ssMisc = new EeUtilServiceImplService(urlMisc);
        miscWs = ssMisc.getEeUtilServiceImplPort();
      } catch (MalformedURLException e) {
        logger.error("No se puede crear la URL del servicio Eeutil Misc " + urlMiscString, e);
        throw new InSideException("No se puede crear la URL del servicio Eeutil Misc ", e);
      }
    }
  }
  ...
  public File convertirTCNAPdf(ApplicationLogin info, File tcnFile, String mime)
    ...
    configure();
    es.mpt.dsic.eeutil.misc.consumer.model.ApplicationLogin credential =
        new es.mpt.dsic.eeutil.misc.consumer.model.ApplicationLogin();
