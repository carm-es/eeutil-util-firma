...
public class EeUtilServiceImpl implements EeUtilService {
  ...
  public String generarCSV(ApplicationLogin info, CSVInfo copia) throws InSideException {
    ...
    try {
      ...
      EstadoInfo estadoInfo = new EstadoInfo();
      throw new InSideException("No se puede obtener el csv de la firma " + t.getMessage(),
          estadoInfo, t.getCause());
    }
  ...
  public CopiaInfo generarCopiaFirmaNormalizada(ApplicationLogin info, CopiaInfo copia,
    ...
    try {
      ...
      logger.warn(
          "Descriptores abiertos inicio generarCopiaFirmaNormalizada: " + FdUtils.getFdOpen());

      if (StringUtils.isEmpty(copia.getIdAplicacion())
          && !CSVUtil.hasScope(copia.getIdAplicacion())) {
        throw new InSideException("No se ha especificado el �mbito", null);
      }

      String rutaLogo = aplicacionContext.getAplicacionInfo().getPropiedades().get("rutaLogo");

      String inputPathFile =
          FileUtil.createFilePath("copiaFirmaNormalizada", copia.getContenido().getContenido());

      entrada = new File(inputPathFile);

      // Se convierte a PDF Es importante para los documentos pdf con campos editables para que al
      // minimizarles
      ...
      pdfReduced = newPdfConversion.copiaPDFReducidoStamper(pdfCopy,
          pdfCopiaAutenticaUtils.createPdfOptionsWithOutPercent(copia.getOpcionesPagina()));

      // TODO Elimninar este codigo para usar el optimizador de jpdfoptimizer o pdfoptimizer de pdf
      // tools
      if (pdfReduced.length() > MAX_FILE_SIZE_BYTES) {
        if (pdfReduced != null && pdfReduced.exists()) {
          FileUtils.forceDelete(pdfReduced);
        }
        pdfReduced = newPdfConversion.shrinkPdf(pdfCopy,
            pdfCopiaAutenticaUtils.createPdfOptions(copia.getOpcionesPagina()));
      }

      // jPdfOptimizer.auditFile(pdfReduced);
      // fileOptimized = jPdfOptimizer.optimize(pdfReduced);
      // fileOptimized = pdfOptimizerPdfTools.optimizePdf(pdfReduced.getAbsolutePath(),
      // PdfOptimize.OPTIMIZATIONPROFILE.eOptimizationProfileWeb);
      ...
      if (pagsBlancas > 0) {
        ...
        conPaginasEnBlanco =
            pdfCopiaAutenticaUtils.insertarPaginasEnBlanco(itextSmaller, pagsBlancas);

        logger
            .debug("Descriptores abiertos despues insertarPaginasEnBlanco: " + FdUtils.getFdOpen());

        // Se estampa la información
        pdfResult = pdfCopiaNormalizadaUtils.copiaAutenticaNormalizada(conPaginasEnBlanco, copia,
            rutaLogo, simple, firmas, pagsBlancas);

        logger.debug(
            "Descriptores abiertos despues copiaAutenticaSimpleFirma: " + FdUtils.getFdOpen());
      } else {
        // Se estampa la información
        pdfResult = pdfCopiaNormalizadaUtils.copiaAutenticaNormalizada(itextSmaller, copia,
            rutaLogo, simple, firmas, pagsBlancas);
      ...
      /*
       * try { if (fileOptimized != null) { FileUtils.forceDelete(fileOptimized); } } catch
       * (IOException e) { logger.error("Error al eliminar fichero:" +
       * fileOptimized.getAbsolutePath()); }
       */
  ...
  public CopiaInfoFirmaSalida generarJustificanteFirma(ApplicationLogin info,
    ...
    try {
      ...
      if (copiaInfoFirma.isIncluirCSV()) {
        csv = obtenerCSV(copiaInfoFirma.getFirma(), copiaInfoFirma.getDocumento(),
      ...
      if (copiaInfoFirma.isIncluirFirmantes()) {
        firmantes = obtenerFirmantes(info.getIdApplicacion(), copiaInfoFirma.getFirma());
      ...
      copiaInfo = this.generarCopiaFirmaComun(info, copiaInfo, firmantes, simple);
      ...
      throw new InSideException("No se puede obtener el justificante de la firma " + t.getMessage(),
          estadoInfo);
    }
  ...
  public CopiaInfo generarInforme(ApplicationLogin login, CopiaInfoExtended copia)
    ...
    try {
      ...
      ListaFirmaInfo firmantes = obtenerFirmantes(login.getIdApplicacion(), copia.getFirma());
      ponerNifsVacios(firmantes);
      ...
      throw new InSideException("No se puede obtener el informe de la firma " + t.getMessage(),
          new EstadoInfo());
    }
  }
   ...
   * @throws InSideException
   */
  private ListaFirmaInfo obtenerFirmantes(String aplicacion, byte[] firma) throws InSideException {
   ...
   */
  /*
   * private boolean esPADES(byte[] firma) { boolean esPADES = false; AOSigner signer =
   * AOSignerFactory.getSigner(firma); if (signer instanceof AOPDFSigner) { esPADES = true; } return
   * esPADES; }
   */
   ...
   * Convierte a PDF
   */
  private File convertir(ApplicationLogin info, File contenido, String mimeType) throws Exception {
    ...
    if (StringUtils.isNotEmpty(mimeType)
        && (mimeType.contentEquals("text-tcn/html") || mimeType.contentEquals("text/tcn"))) {
  ...
  private CopiaInfo generarCopiaFirmaComun(ApplicationLogin login, CopiaInfo copia,
    ...
    try {
      ...
      String inputPathFile =
          FileUtil.createFilePath("copiaFirma", copia.getContenido().getContenido());
      ...
      /// ***** necesario para pdf con campos editables hay que trabajar sobre una copia de el para
      /// no perder el valor de esos campos **///
      ...
      if (pagsBlancas > 0) {
        ...
        pdfResult = pdfCopiaAutenticaUtils.copiaAutenticaSimpleFirma(conPaginasEnBlanco, copia,
            simple, firmas, pagsBlancas, logoProperties);

        logger.debug(
            "Descriptores abiertos despues copiaAutenticaSimpleFirma: " + FdUtils.getFdOpen());
      } else {
        // Se estampa la información
        pdfResult = pdfCopiaAutenticaUtils.copiaAutenticaSimpleFirma(itextSmaller, copia, simple,
