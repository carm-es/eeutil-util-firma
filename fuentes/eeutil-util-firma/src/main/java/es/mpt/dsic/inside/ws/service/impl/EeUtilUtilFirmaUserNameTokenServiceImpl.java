...
public class EeUtilUtilFirmaUserNameTokenServiceImpl
  ...
  public CopiaInfo generarCopiaFirmaNormalizada(CopiaInfo copia, ListaFirmaInfo firmas,
    ...
    try {
      ...
      if (pagsBlancas > 0) {
        ...
        pdf = pdfCopiaNormalizadaUtils.insertarPaginasEnBlanco(pdf, pagsBlancas);
      }
      // Se estampa la información
      pdfResult = pdfCopiaNormalizadaUtils.copiaAutenticaNormalizada(pdf, copia, rutaLogo, simple,
          firmas, pagsBlancas);

      copia.getContenido().setContenido(IOUtil.getBytesFromObject(pdfResult));
  ...
  public CopiaInfoFirmaSalida generarJustificanteFirma(CopiaInfoFirma copiaInfoFirma,
    ...
    try {
      ...
      copiaInfo = this.generarCopiaFirmaComun(copiaInfo, firmantes, simple);
  ...
  public CopiaInfo generarInforme(CopiaInfoExtended copia) throws InSideException {
    try {
      ...
      ponerNifsVacios(firmantes);
      CopiaInfo retorno = generarCopiaFirmaComun(copia, firmantes, copia.isSimple());
  ...
  private String obtenerCSV(byte[] firma, byte[] documentoFirmado)
    ...
    try {
      processor.processSign(firma, documentoFirmado,
   ...
   * @throws InSideException
   */
  private ListaFirmaInfo obtenerFirmantes(byte[] firma) throws InSideException {
    InformacionFirmaAfirma info;
    try {
      info = afirmaService.obtenerInformacionFirma(
          credentialUtil.getCredentialEeutilUserToken(wsContext).getIdApplicacion(), firma, true,
          false, false, null);
  ...
  private CopiaInfo generarCopiaFirmaComun(CopiaInfo copia, ListaFirmaInfo firmas, boolean simple)
      throws InSideException {
    File pdf = null;
    File entrada = null;
    ...
    try {
      ...
      if (pagsBlancas > 0) {
        ...
        File conPaginasEnBlanco =
            pdfCopiaAutenticaUtils.insertarPaginasEnBlanco(pdfReduced, pagsBlancas);
        ...
        // Se estampa la información
        pdfResult = pdfCopiaAutenticaUtils.copiaAutenticaSimpleFirma(pdfReduced, copia, simple,
  ...
  public String generarHash(@XmlMimeType("application/octet-stream") DataHandler data,
    ...
    } else if (algoritmo.trim().toUpperCase().equals(ALGORITMO_SHA256)
    ...
    } else if (algoritmo.trim().toUpperCase().equals(ALGORITMO_SHA384)
    ...
    } else if (algoritmo.trim().toUpperCase().equals(ALGORITMO_SHA512)
