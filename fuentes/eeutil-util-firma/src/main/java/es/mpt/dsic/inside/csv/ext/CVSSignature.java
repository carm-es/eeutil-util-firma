...
public class CVSSignature {
   ...
   * @see #setPattern(String)
   */
  public String getCVSTextOM()
      throws CSVCantBeGeneratedException, AOInvalidFormatException, IOException {
    ...
    // Documento firmado.
    /*
     * byte[] signedData = null; try { signedData = getSignedData (); } catch (Exception e) { throw
     * new CSVCantBeGeneratedException ("No se puede extraer el contenido firmado" , e); }
     */
    ...
    try {
      ...
      sha256DocumentoFirmado =
          CSVUtil.generateDigest(this.signProcessor.getSignedDocument(), "SHA-256");
    } catch (NoSuchAlgorithmException e) {
      throw new CSVCantBeGeneratedException(
          "No se puede generar la huella para el documento firmado", e);
    }
    ...
    try {
      ...
      throw new CSVCantBeGeneratedException(
          "No se puede generar la huella para los pkcs de la firma", e);
    }
  ...
  /**
   * Procesa todos los elementos del CVS de la firma y los almacena internamente para no repetir la
   * operaci&oacute;n, salvo que se cambie de patr&oacute;n.
   * 
   * @throws Exception
   */
  private void processAllElements() throws Exception {
    ...
    if (this.patternElements == null) {
      this.patternElements =
          new PatternElement[] {new PatternElement(PatternElementType.PKCS1XORHASH, null)};
    }
  ...
  /**
   * Obtiene la cadena de texto correspondiente a un patr&oacute;n. Si es un texto el propio texto,
   * si es un patr&oacute;n de hash calcula el hash de la firma con el algoritmo especificado (o el
   * por defecto si no se indic&oacute;), etc.
   * 
  ...
  /**
   * Calcula el valor del patr&oacute;n por defecto de la firma. Este valor es el hash en base 64
   * del XOR de los PKCS#1 de la firma.
   * 
